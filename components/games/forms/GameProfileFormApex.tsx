...
import { APEX_CHARACTERS, APEX_RANKS } from "@/constants/gameAssets";
...
export function ApexGameProfileForm({ mode, initial, onSave, onDelete }: Props) {
  const [pending, start] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const [currentRank, setCurrentRank] = useState<string>(() => normalizeApexRank(initial?.currentRank));
  const [highestRank, setHighestRank] = useState<string>(() => normalizeApexRank(initial?.highestRank));
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      <section className="space-y-3">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <RankGrid ranks={APEX_RANKS} selected={currentRank} onSelect={setCurrentRank} name="currentRank" slug="apex-legends" />
          </div>
          <div className="space-y-2">
      ...
      <section className="space-y-3">
        ...
        <Textarea
          ...
          placeholder="得意な立ち回りやよく使う構成などを自由に書けます。"
          className="text-sm"
        />
      </section>
...
function normalizeApexRank(raw?: string | null): string {
  if (!raw) return "";
  const lower = raw.toLowerCase();
  const tiers = APEX_RANKS.map((r) => r.value.toLowerCase());
  const found = tiers.find((t) => lower.includes(t));
  return found ? APEX_RANKS.find((r) => r.value.toLowerCase() === found)!.value : raw;
}
