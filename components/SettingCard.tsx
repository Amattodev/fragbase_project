        ...
        {settings && settings.length > 0 ? (
          <>
            {settings.map((setting) => (
              <Link
                key={setting.id}
                href={`/settings/${setting.id}?returnUrl=${encodeURIComponent(
                ...
                <Card className="bg-[#2B2B2B] w-full max-w-3xl">
                  <CardContent className="p-4 ">
                    <h3 className="text-lg font-semibold text-[#F9F9F9]">
                      {" "}
                      {setting.gameTitle} / {setting.character} /{" "}
                      {getFpsExperienceLabel(setting.fpsExperience)}
                    </h3>
                    <p className="text-[#BBBBBB] text-sm">
                      ゲーム内感度：{setting.sensitivity} / DPI：
                      {getDpiLabel(setting.dpi)} / デバイス：
                      {getDeviceLabel(setting.device)}
                    </p>
