...
export default async function UserGameProfileViewPage({
  params,
}: {
    ...
    <div className="space-y-4 py-4">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        ...
        <div className="space-y-4 lg:col-span-9">
          <GameMainCharactersCard
            slug={game.slug}
            mainCharacters={profile.mainCharacters ?? undefined}
          />
        </div>
      </div>
      ...
      {!profile.currentRank &&
        ...
        !(profile.notes && profile.notes.trim().length) && (
          <p className="text-sm text-muted-foreground">
            まだ情報が登録されていません。編集から追加しましょう。
          </p>
        )}
    </div>
  );
