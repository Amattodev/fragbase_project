...
import { MAX_VIDEO_FILE_SIZE_R2 } from "@/constants/limits";
import { DEFAULT_R2_PUBLIC_URL } from "@/constants/upload";

// 最大ファイルサイズ（R2側の制限に合わせる）
const MAX_FILE_SIZE = MAX_VIDEO_FILE_SIZE_R2;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 公開URLを生成（環境変数から取得）
    const publicUrl =
      process.env.CLOUDFLARE_R2_PUBLIC_URL || process.env.NEXT_PUBLIC_CLOUDFLARE_R2_PUBLIC_URL || DEFAULT_R2_PUBLIC_URL;
